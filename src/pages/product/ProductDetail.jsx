...
export default function ProductDetail() {
  ...
  // Fetch product from API by slug (falls back to local JSON)
  useEffect(() => {
    if (!slug) {
      setProduct(null)
      return
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (productData) {
          ...
          setRelatedProducts(
            categoryProducts.filter(p => p.id !== productData.id).slice(0, 4)
          )
        } else {
          setProduct(null)
        }
            ...
            {images.length > 1 && (
              <div className={styles.thumbnails}>
                {images.map((img, idx) => (
                  ...
                  >
                    <img src={img || '/placeholder.jpg'} alt={`${product.name} view ${idx + 1}`} onError={(e) => e.target.style.display = 'none'} />
                  </button>
                ))}
              </div>
            )}
